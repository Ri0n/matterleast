...
PostCacheStore::Limits configuredLimits()
{
    ...
    limits.maxBytes = std::max<qint64>(
        MiB,
        static_cast<qint64>(optionInt(POST_CACHE_DISK_MAX_MB,
                                     POST_CACHE_DISK_MAX_MB_DEFAULT)) * MiB);
    ...
    limits.maxChannelIdleMs = std::max<qint64>(
        60LL * 60 * 1000,
        static_cast<qint64>(optionInt(POST_CACHE_DISK_CHANNEL_IDLE_HOURS,
...
PostCacheService::PostCacheService(QString databasePath)
{
    ...
    const auto watchLimit = [this](const char* key, int defaultValue) {
        ...
        QObject::connect(option, &MLOptionObject::changed, &callbackContext,
                         [this](const QVariant&) {
            scheduleConfiguredLimitsUpdate();
        });
    };
...
void PostCacheService::scheduleConfiguredLimitsUpdate()
{
    if (limitsUpdatePending) {
        return;
    }
    limitsUpdatePending = true;
    QMetaObject::invokeMethod(
        &callbackContext,
        [this] {
            limitsUpdatePending = false;
            if (!worker) {
                return;
            }
